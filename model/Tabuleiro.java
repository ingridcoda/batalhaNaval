...
public class Tabuleiro extends JPanel implements MouseListener{
	...
	protected static Matriz[][] matrizControleJogadorVez = Matriz.inicializaMatriz(Tabuleiro.matrizControleJogadorVez);
	private Color cor;
	
	public Tabuleiro(Matriz[][] matrizControleJogadorVez, int x, int y){
		matrizControleJogadorVez = Matriz.atribuiMatriz(Tabuleiro.matrizControleJogadorVez, matrizControleJogadorVez);
	...
	public void paintComponent(Graphics g){
		...
		while (j < 15){ 
			for(i = 0; i < 15; i++){
				...
				if(matrizControleJogadorVez[i][j].statusAcerto == true){
					g2d.setColor(Color.RED);
					g2d.fill(new Rectangle2D.Double(x, y, 20, 20));
					repaint();
				} else if(matrizControleJogadorVez[i][j].tipoArma >= 1){
	...
	public static Matriz[][] getMatrizControle(){
		return matrizControleJogadorVez;
	}


	@Override
	public void mouseClicked(MouseEvent mouse) {
		new ControladorMatriz(matrizControleJogadorVez, mouse);
